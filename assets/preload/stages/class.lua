...
runHaxeCode([[game.eventPushedMap.set('addCharacter',true)]])
end

function onCreatePost()
sprite(-641.35,-209,'bg',false)
triggerEvent('addCharacter','najimi','-41,247')
runHaxeCode([[getVar('najimi').visible = false]])
...
runHaxeCode([[if(getVar('najimi').animation.curAnim.name == 'idle'){getVar('najimi').playAnim('blink',true,false,getVar('najimi').animation.curAnim.curFrame)}]])
if not blinko then
runHaxeCode([[if(getVar('najimi').animation.curAnim.name == 'blink'){getVar('najimi').playAnim('idle',true,false,getVar('najimi').animation.curAnim.curFrame)}]])
end
end
...
if curBeat == 122 then
runHaxeCode([[getVar('najimi').visible = true]])
runTimer('blink3',5,0)
end
...
runHaxeCode([[FlxTween.color(getVar('najimi'),6,0xffffffff,0xff000000)]])

doTweenAlpha('iconP1','iconP1',0,1)
doTweenAlpha('iconP2','iconP2',0,1)
doTweenAlpha('healthBar','healthBar',0,1)
doTweenAlpha('healthBarBG','healthBarBG',0,1)
...
runHaxeCode([[FlxTween.color(getVar('najimi'),3,0xff000000,0xffffffff)]])
end

if curBeat == 205 then
doTweenAlpha('iconP1','iconP1',1,1)
doTweenAlpha('iconP2','iconP2',1,1)
doTweenAlpha('healthBar','healthBar',1,1)
doTweenAlpha('healthBarBG','healthBarBG',1,1)
